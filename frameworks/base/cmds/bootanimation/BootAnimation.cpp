...
static const char SYSTEM_BOOTVIDEO_FILE[] = "/system/media/video/bootvideo.mp4";
static const char SYSTEM_SHUTDOWNVIDEO_FILE[] = "/system/media/video/shutdownvideo.mp4";
...
status_t BootAnimation::readyToRun() {
    ...
    //ALOGD("ykq--mShuttingDown = %d ", mShuttingDown);
    if (access(!mShuttingDown ? SYSTEM_BOOTVIDEO_FILE : SYSTEM_SHUTDOWNVIDEO_FILE, R_OK) == 0) {
        mVideo = true;
        mVideoFileName = !mShuttingDown ? SYSTEM_BOOTVIDEO_FILE : SYSTEM_SHUTDOWNVIDEO_FILE;
        //ALOGD("ykq--mVideoFileName string is %s, length is %zu", mVideoFileName.string(), mVideoFileName.length());
    }
    /* add end */
 
    //ALOGD("ykq--Get Surface mWidth is %d,mHeight is %d",mWidth,mHeight);
    return NO_ERROR;
}
...
{
    bool r;
    // The priority from large to small is: video - movie - android-logo
    if(mVideo){
		r = video();
...
{
    //ALOGD("ykq--video func in.");
 
    //check video file leghth
    if (mVideoFileName.length() == 0) {
        ALOGD("mVideoFileName.length = 0 !!!");
        return false;
    }
 
    mVideofd = open(mVideoFileName.string(), O_RDONLY);
    if(mVideofd == -1){
        //ALOGD("ykq--can not open mp4 bootvideo resource !!!");
        return false;
    }
    ...
    t.setPosition(mFlingerSurfaceControl, mWidth, 0);
    //t.setMatrix(mFlingerSurfaceControl, 0, 1 / asp, -asp, 0);
    t.apply();
#endif
 
    sp<MediaPlayer> mp = new MediaPlayer();
    mp->reset();
    mp->setDataSource(mVideofd, 0, 0x7ffffffffffffffLL);
    //mp->setLooping(true);
    mp->setVideoSurfaceTexture(mFlingerSurface->getIGraphicBufferProducer());
    mp->prepare();
 
#if 0 //set video scale mode by invoke, you can refer to [setVideoScalingMode() @ frameworks\base\media\java\android\media\MediaPlayer.java]
    Parcel* _request = new Parcel;
    Parcel* _reply = new Parcel;
    const String16 _token = String16("android.media.IMediaPlayer");
    _request->writeInterfaceToken(_token);
    _request->writeInt32(6);
    _request->writeInt32(2);
    mp->invoke(*_request, _reply);
 
#else //use egl setSize() & setPosition() to set SurfaceFlinger size and position.
    int _width = 0;
    int _height = 0;
    mp->getVideoHeight(&_height);
    mp->getVideoWidth(&_width);
    //ALOGE("ykq--BootAnimation::video width is %d, height is %d .", _width, _height);
    if (_width != 0 && _height != 0) {
        float devicetRatio = float(mWidth) / float(mHeight);
        float videoRatio = float(_width) / float(_height);
        if (videoRatio >= devicetRatio) {
            float wScale = float(_width) / float(mWidth);
            SurfaceComposerClient::Transaction t;
            t.setSize(mFlingerSurfaceControl, mWidth, int(float(_height)/wScale));
            if (mHeight > int(float(_height)/wScale)) {
                int yT = (mHeight - int(float(_height)/wScale)) / 2;
                t.setPosition(mFlingerSurfaceControl, 0, yT);
            }
            t.apply();
        } else {
            float hScale = float(_height) / float(mHeight);
            SurfaceComposerClient::Transaction t;
            t.setSize(mFlingerSurfaceControl, int(float(_width)/hScale), mHeight);
            if (mWidth > int(float(_width)/hScale)) {
                int xT = (mWidth - int(float(_width)/hScale)) / 2;
                t.setPosition(mFlingerSurfaceControl, xT, 0);
            }
            t.apply();
        }
    }
    ...
    while(true) {
        ...
        usleep(CHECK_DELAY);
		//ALOGD("ykq--while2");
        if(!mp->isPlaying()) { //show launcher after play complete.
            checkExit();
        }
		//ALOGD("ykq--while3");
    }
...
bool BootAnimation::movie()
{
	...
	//ALOGD("ykq--animation load complete");
...
bool BootAnimation::playAnimation(const Animation& animation)
{
    ...
    for (size_t i=0 ; i<pcount ; i++) {
		//ALOGE("ykq--ykq = %d ", i); Folder traversal
        ...
        for (int r=0 ; !part.count || r<part.count ; r++) {
			//ALOGE("ykq--ykq2 = %d ", r); repeat times
            ...
            for (size_t j=0 ; j<fcount && (!exitPending() || part.playUntilComplete) ; j++) {
				//ALOGE("ykq--ykq3 = %d ", 3);  Every frame
