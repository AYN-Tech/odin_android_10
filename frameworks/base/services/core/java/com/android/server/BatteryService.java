...
public final class BatteryService extends SystemService {
    ...
    private void shutdownIfOverTempLocked() {
        // shut down gracefully if temperature is too high (> 68.0C by default(680))
        // wait until the system has booted before attempting to display the
        // shutdown dialog.
        if (mHealthInfo.batteryTemperature > mShutdownBatteryTemperature) {
			Slog.e(TAG, "关机：电池温度batteryTemperature" + mHealthInfo.batteryTemperature + ", 超过:" + mShutdownBatteryTemperature);
			/*
            mHandler.post(new Runnable() {
                @Override
                public void run() {
