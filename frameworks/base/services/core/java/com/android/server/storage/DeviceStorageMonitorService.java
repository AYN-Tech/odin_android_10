...
public class DeviceStorageMonitorService extends SystemService {
    ...
    private static final long DEFAULT_CHECK_INTERVAL = DateUtils.MINUTE_IN_MILLIS/2;
    ...
    private static final int FORCE_MODE_THRESHOLD = 150 * 1024 * 1024;
    private boolean mForceModeFlag = false;
    private Intent mDataConnectionDisabled;
    private Intent mDataConnectionEnabled;
    ...
    private void check() {
        ...
        for (VolumeInfo vol : storage.getWritablePrivateVolumes()) {
			...
			//add by zm
			if (file != null && file.equals(Environment.getDataDirectory())) {
				if (usableBytes < FORCE_MODE_THRESHOLD) {
					if ((Settings.Global.getInt(mResolver, Settings.Global.LOW_MEMORY_MODE, 0)) == 0) {
						Settings.Global.putInt(mResolver, Settings.Global.LOW_MEMORY_MODE, 1);
						mHandler.removeMessages(DEVICE_MEMORY_CRITICAL_LOW);
						mHandler.sendMessageDelayed(mHandler.obtainMessage(DEVICE_MEMORY_CRITICAL_LOW),300);
					}
					...
					if ((Settings.Global.getInt(mResolver, Settings.Global.LOW_MEMORY_MODE, 0)) != 0) {
						Settings.Global.putInt(mResolver, Settings.Global.LOW_MEMORY_MODE, 0);
						leaveLowStorage("com.android.settings.OutSpaceDialog");
					}
    ...
    public DeviceStorageMonitorService(Context context) {
        ...
        mDataConnectionEnabled = new Intent("storageforcemode.enable.dataconnection");
        ...
        mHandler = new Handler(mHandlerThread.getLooper()) {
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
					...
					case DEVICE_MEMORY_CRITICAL_LOW:// add by zm 
						if (!isLowStorageOn("com.android.settings.OutSpaceDialog") && ActivityManager.isSystemReady() ) {
							try {
								Intent outofSpaceIntent = new Intent("com.retrostation.action.OUT_OF_SPACE");
								outofSpaceIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
								mContext.startActivity(outofSpaceIntent);
							} catch (ActivityNotFoundException exception) {
								Slog.w(TAG, "Activity com.android.settings.OutSpaceDialog not found.");
							}
						}
						return; // add by zm end
                }
            }
        };
    }
    ...
    public void onStart() {
        ...
        intentFilter.addAction("android.intent.action.LOW_MEMORY_NOTIFICATION");
        mContext.registerReceiver(mLowmemoryReceiver, intentFilter);
        //add by zm end
    }
    ...
    private void updateNotifications(VolumeInfo vol, int oldLevel, int newLevel) {
        ...
        if (State.isEntering(State.LEVEL_LOW, oldLevel, newLevel)) {
            ...
            //add by zm
            Intent mIntent = new Intent(LowMemoryAction);
            mIntent.putExtra("lowMemIntent",lowMemIntent);
            ...
            if (StorageManager.UUID_DEFAULT.equals(uuid)) {
                ...
                title = context.getText(com.android.internal.R.string.low_internal_storage_view_title2);
                details = context.getText(isBootImageOnDisk()
                        ? com.android.internal.R.string.low_internal_storage_view_text1
                        : com.android.internal.R.string.low_internal_storage_view_text_no_boot);
                intent = PendingIntent.getBroadcastAsUser(context, 0,  mIntent, 0, UserHandle.CURRENT);
            } else {
                title = context.getText(com.android.internal.R.string.low_internal_storage_view_title);
                details = context.getText(
                        com.android.internal.R.string.low_internal_storage_view_text);
                intent = PendingIntent.getActivityAsUser(context, 0, lowMemIntent, 0,null, UserHandle.CURRENT);
            }
    ...
    //add by zm
    private AlertDialog mLowMemoryDialog;
    private static final String LowMemoryAction = "android.intent.action.LOW_MEMORY_NOTIFICATION";

    private BroadcastReceiver mLowmemoryReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if(LowMemoryAction.equals(intent.getAction())){
                final Intent lowMemIntent = (Intent)intent.getExtra("lowMemIntent");
                lowMemoryNotify(lowMemIntent);
            }
            //else{
            //    sendNotification();
            //}
    ...
    private void lowMemoryNotify(final Intent intent){
        if (mLowMemoryDialog != null && mLowMemoryDialog.isShowing()) {
            return;
        }
        AlertDialog.Builder mBuilder = new AlertDialog.Builder(mContext);
        mBuilder.setTitle(getContext().getText(com.android.internal.R.string.low_internal_storage_view_title2))
                .setMessage(getContext().getText(com.android.internal.R.string.low_internal_storage_view_text2))
                .setPositiveButton(getContext().getText(com.android.internal.R.string.low_internal_storage_btn_view_storage).toString().toUpperCase(),
                    new OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            Intent actionIntent = new Intent(StorageManager.ACTION_MANAGE_STORAGE);
                            actionIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                            getContext().startActivity(actionIntent);
                        }
                    })
                  .setNegativeButton(getContext().getText(com.android.internal.R.string.cancel).toString().toUpperCase(),
                    new OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            dialog.dismiss();
        ...
        mLowMemoryDialog.getWindow().setType(
                WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
        mLowMemoryDialog.getWindow().getAttributes().privateFlags
    ...
    private void sendDataDisabledNotification() {
        Slog.d("zhouming", "----disableDataConnection ");
        getContext().removeStickyBroadcastAsUser(mDataConnectionDisabled, UserHandle.ALL);
        getContext().removeStickyBroadcastAsUser(mDataConnectionEnabled, UserHandle.ALL);

        getContext().sendStickyBroadcastAsUser(mDataConnectionDisabled, UserHandle.ALL);
    }

    private void sendDataEnabledNotification() {
        Slog.d("zhouming", "----enableDataConnection  ");
        getContext().removeStickyBroadcastAsUser(mDataConnectionDisabled, UserHandle.ALL);
        getContext().removeStickyBroadcastAsUser(mDataConnectionEnabled, UserHandle.ALL);

        getContext().sendBroadcastAsUser(mDataConnectionEnabled, UserHandle.ALL);
    }
    public boolean isLowStorageOn(String taskName) {
        ...
        while (iterator.hasNext()) {
            ...
            if (isRunningTask) {
                Slog.i(TAG, "isLowStorageOn task " + taskName + " is running");
                return true;
            }
        }
        Slog.i(TAG, "isLowStorageOn task " + taskName + " return false");
        return false;
    }
    ...
    private void leaveLowStorage(String taskName) {
        ...
        while (iterator.hasNext()) {
            ...
            if (isRunningTask) {
                Slog.i(TAG, "remove task " + taskName);
                try {
                    ActivityTaskManager.getService().removeTask(runningTaskInfo.id);
