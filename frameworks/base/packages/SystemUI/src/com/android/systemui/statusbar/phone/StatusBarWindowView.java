...
public class StatusBarWindowView extends FrameLayout {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
		...
		if(controllerStyleMatch(event) && down){
			return tryOpenScreen();
        }
	...
	private boolean tryOpenScreen(){
		PowerManager pm = (PowerManager) getContext().getSystemService(Context.POWER_SERVICE);
		if(pm.isScreenOn()){
			mService.animateCollapsePanels(CommandQueue.FLAG_EXCLUDE_NONE, true);
			return true;
		}else{
	...
	private boolean controllerStyleMatch(KeyEvent event){
		boolean style = OdinSettings.flipButtonLayout.value();
		if(style){
			return event.getKeyCode() == KeyEvent.KEYCODE_BUTTON_B;
		} else {
			return event.getKeyCode() == KeyEvent.KEYCODE_BUTTON_A;
		}
	}
