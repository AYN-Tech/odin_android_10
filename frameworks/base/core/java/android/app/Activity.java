...
public class Activity extends ContextThemeWrapper
    ...
    //add by zm begin
    private static final long LIMIT_SPACE = 153600L;
    private static java.util.Set<String> certificatedSet = new java.util.HashSet<String>();
    static {
        certificatedSet.add("com.android.settings");
        certificatedSet.add("com.retrostation.odinsettings");
        certificatedSet.add("com.google.android.apps.nbu.files");
        certificatedSet.add("com.retrostation.odinlauncher");
		certificatedSet.add("com.retrostation.gameassistant");
		certificatedSet.add("com.retrostation.upgrade");
		certificatedSet.add("com.android.documentsui");
    }
    private boolean isOutOfSpace() {
        StatFs stat = new StatFs(Environment.getDataDirectory().getPath());
        return ((long)stat.getBlockSize() * (long)stat.getAvailableBlocks() / 1024) < LIMIT_SPACE;
    }
    private boolean isCertificated(String packageName) {
        return certificatedSet.contains(packageName);
    }
     //add by zm end
    ...
    public void startActivityForResult(@RequiresPermission Intent intent, int requestCode,
        ...
        if (isOutOfSpace()) {
            ...
            List<ResolveInfo> resolvedInfos = pm.queryIntentActivities(intent,0);
            for (ResolveInfo resolveInfo : resolvedInfos) {
                if (isCertificated(resolveInfo.activityInfo.packageName)) {
                    isCertificated = true;
                    break;
                }
            }
            if (intent != null) {
                boolean isUninstallAction = false;
                boolean isUninstallClass = false;
                String action = intent.getAction();
                if (action != null && (action.equals(Intent.ACTION_UNINSTALL_PACKAGE) || action.equals(Intent.ACTION_DELETE))) {
                    isUninstallAction = true;
                }
                ComponentName componentName = intent.getComponent();
                if (componentName != null) {
                    String className = componentName.getClassName();
                    if (className != null && className.equals("com.android.packageinstaller.UninstallUninstalling")) {
                        isUninstallClass = true;
                    }
                }
                if (isUninstallAction || isUninstallClass) {
                    isCertificated = true;
                }
            }
            if (!isCertificated) {
                intent = new Intent("com.retrostation.action.OUT_OF_SPACE");
            }
        }
        //add by zm end
        if (mParent == null) {
