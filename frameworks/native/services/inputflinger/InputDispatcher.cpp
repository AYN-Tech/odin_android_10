...
void InputDispatcher::userActivityForVirtualCursor() {
	nsecs_t eventTime = now();
   	int32_t	userActivityEventType = USER_ACTIVITY_EVENT_OTHER;	

	mLock.lock();
    mPolicy->pokeUserActivity(eventTime, userActivityEventType);
	mLock.unlock();
