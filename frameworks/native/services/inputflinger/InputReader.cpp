...
InputDevice* InputReader::createDeviceLocked(int32_t deviceId, int32_t controllerNumber,
	...
	if (classes & INPUT_DEVICE_CLASS_CURSOR_ICON) {
        device->addMapper(new CursorIconInputMapper(device));
	}
...
// --- CursorIconMotionAccumulator ---

CursorIconMotionAccumulator::CursorIconMotionAccumulator() {
    clearRelativeAxes();
}

void CursorIconMotionAccumulator::reset(InputDevice* device) {
    clearRelativeAxes();
}

void CursorIconMotionAccumulator::clearRelativeAxes() {
    mRelX = 0;
    mRelY = 0;
}

void CursorIconMotionAccumulator::process(const RawEvent* rawEvent) {
    if (rawEvent->type == EV_REL) {
        switch (rawEvent->code) {
            case REL_RX:
                mRelX = rawEvent->value;
...
// --- CursorIconFadeAccumulator ---

CursorIconFadeAccumulator::CursorIconFadeAccumulator() {
    clearState();
}

void CursorIconFadeAccumulator::reset(InputDevice* device) {
    clearState();
}

void CursorIconFadeAccumulator::clearState() {
    mNeedTofade = false;
}

void CursorIconFadeAccumulator::process(const RawEvent* rawEvent) {
    if (rawEvent->type == EV_REL) {
        switch (rawEvent->code) {
            case REL_MISC:
                mNeedTofade = rawEvent->value > 0;
                break;
        }
    }
}

void CursorIconFadeAccumulator::finishSync() {
    clearState();
}


void CursorIconMotionAccumulator::finishSync() {
    clearRelativeAxes();
}
...
void SingleTouchMotionAccumulator::reset(InputDevice* device) {
	...
	mAbsMTTrackingId = device->getAbsoluteAxisValue(ABS_MT_TRACKING_ID);
}
...
void SingleTouchMotionAccumulator::process(const RawEvent* rawEvent) {
    if (rawEvent->type == EV_ABS) {
		...
		case ABS_MT_TRACKING_ID:
            mAbsMTTrackingId = rawEvent->value;
			break;
        }
    }
}
...
// --- CursorIconInputMapper ---

CursorIconInputMapper::CursorIconInputMapper(InputDevice* device) : 
        InputMapper(device) {
}

CursorIconInputMapper::~CursorIconInputMapper() {
}

uint32_t CursorIconInputMapper::getSources() {
    return mSource;
}

void CursorIconInputMapper::populateDeviceInfo(InputDeviceInfo* info) {
    InputMapper::populateDeviceInfo(info);
...
void CursorIconInputMapper::dump(std::string& dump) {
    dump += INDENT2 "CursorIcon Input Mapper:\n";
    dumpParameters(dump);
...
void CursorIconInputMapper::configure(nsecs_t when, const InputReaderConfiguration* config,
                                  uint32_t changes) {
    InputMapper::configure(when, config, changes);
...
void CursorIconInputMapper::configureParameters() {
    mParameters.mode = Parameters::MODE_POINTER;
...
void CursorIconInputMapper::dumpParameters(std::string& dump) {
        dump += INDENT3 "Parameters:\n";
...
void CursorIconInputMapper::reset(nsecs_t when) {
    mButtonState = 0;
    mDownTime = 0;
    ...
    mCursorIconMotionAccumulator.reset(getDevice());
    mCursorScrollAccumulator.reset(getDevice());
	mCursorIconFadeAccumulator.reset(getDevice());

    InputMapper::reset(when);
}

void CursorIconInputMapper::process(const RawEvent* rawEvent) {
    mCursorButtonAccumulator.process(rawEvent);
    mCursorIconMotionAccumulator.process(rawEvent);
    mCursorScrollAccumulator.process(rawEvent);
	mCursorIconFadeAccumulator.process(rawEvent);

	ALOGD("CursorIconInputMapper::process rawEvent");
    if (rawEvent->type == EV_SYN && rawEvent->code == SYN_REPORT) {
        sync(rawEvent->when);
    }
}

void CursorIconInputMapper::sync(nsecs_t when) {
    float realX = mCursorIconMotionAccumulator.getRelativeX();
    float realY = mCursorIconMotionAccumulator.getRelativeY();
	bool fade = mCursorIconFadeAccumulator.getNeedToFade();
    float x, y, offsetX, offsetY;
    mPointerController->getPosition(&x, &y);
	
    offsetX = realX - x;
    offsetY = realY - y;
    bool moved = offsetX != 0 || offsetY != 0;

	if (moved) {
		ALOGD("CursorIconInputMapper::sync deltaX : %f  deltaY : %f",realX,realY);
		mPointerController->setPresentation(
                    PointerControllerInterface::PRESENTATION_POINTER);
		mPointerController->setPosition(realX, realY);
		mPointerController->updatePointerIcon(1000);
		mPointerController->unfade(PointerControllerInterface::TRANSITION_IMMEDIATE);
		getListener()->userActivityForVirtualCursor();
	}
	if (fade) {
	...
	mCursorIconFadeAccumulator.finishSync();
    mCursorIconMotionAccumulator.finishSync();
}

int32_t CursorIconInputMapper::getScanCodeState(uint32_t sourceMask, int32_t scanCode) {
...
void CursorIconInputMapper::fadePointer() {
    if (mPointerController != NULL) {
        mPointerController->fade(PointerControllerInterface::TRANSITION_GRADUAL);
    }
}
...
void TouchInputMapper::cookAndDispatch(nsecs_t when) {
    ...
    if (initialDown || buttonsPressed) {
        ...
        if (mDeviceMode == DEVICE_MODE_DIRECT && !isVritualMouseId()) {
...
bool SingleTouchInputMapper::isVritualMouseId(){
    int32_t id = mSingleTouchMotionAccumulator.getAbsoluteTrackingId();
	//ALOGD("TOUTCH ID is %d",id);
	if(id >= 100000001 && id <= 100000006)
		return true;
...
bool MultiTouchInputMapper::isVritualMouseId() {
	uint32_t rsMappingSoltIdIndex = 10;
	uint32_t realTouchSoltIdIndex = 0;
	int32_t rsMappingSoltId = mMultiTouchMotionAccumulator.getSlot(rsMappingSoltIdIndex)->getTrackingId();
	//int32_t realTouchSoltId = mMultiTouchMotionAccumulator.getSlot(realTouchSoltIdIndex)->getTrackingId();
	bool isRsMappingSoltIdUse = mMultiTouchMotionAccumulator.getSlot(rsMappingSoltIdIndex)->isInUse();
	bool isRealTouchSoltIdUse = mMultiTouchMotionAccumulator.getSlot(realTouchSoltIdIndex)->isInUse();
	//ALOGD("Multi TOUTCH rsMappingSoltId is %d realTouchSoltId is %d ",rsMappingSoltId,realTouchSoltId);
	//ALOGD("Multi TOUTCH isRsMappingSoltIdUse is in use %d isRealTouchSoltIdUse is in use %d ",isRsMappingSoltIdUse,isRealTouchSoltIdUse);
	if ((rsMappingSoltId >= 100000001 && rsMappingSoltId <= 100000006) && isRsMappingSoltIdUse && !isRealTouchSoltIdUse)
		return true;
