...
public abstract class RecentsView<T extends BaseActivity> extends PagedView implements Insettable,
    ...
    private void removeTask(Task task, int index, PendingAnimation.OnEndListener onEndListener,
        ...
        if (task != null) {
			...
			if(isShouldForceKill(packageName)){
				am.forceStopPackage(packageName);
			}			
			mModel.onTaskRemoved(task.key.id);
            if (shouldLog) {
	...
	private boolean isShouldForceKill(String packageName){
		if(packageName == null){
			return false;
		}
		if(packageName.equals("com.baidu.otasdk") || packageName.equals("com.odin.ota") || packageName.equals("com.baidu.installerplugin")){
			return false;
		}
		return true;
	}
        ...
        mPendingAnimation.addEndListener((onEndListener) -> {
            if (onEndListener.isSuccess) {
				...
				for(int j = 0; j < count; j++){
					ActivityManager am = (ActivityManager) mActivity.getSystemService(Context.ACTIVITY_SERVICE);
					TaskView taskView = getTaskViewAt(j);
					Task task = taskView.getTask();
					if(task != null){
						am.forceStopPackage(task.key.getComponent().getPackageName());
					}
				}
                removeTasksViewsAndClearAllButton();
                startHome();
