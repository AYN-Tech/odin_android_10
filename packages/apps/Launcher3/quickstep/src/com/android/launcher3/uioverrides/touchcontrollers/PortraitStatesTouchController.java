...
public class PortraitStatesTouchController extends AbstractStateChangeTouchController {
	...
	private static final int SHOW_RECENTS_Y_POINT = 800;
    ...
    protected boolean canInterceptTouch(MotionEvent ev) {
		...
		//Log.d(TAG, "canInterceptTouch x = " + ev.getRawX() + "  y = " + ev.getRawY());

		if(!isShouldDrawOutOverview(ev)){
			return false;
		}
	...
	private boolean isShouldDrawOutOverview(MotionEvent ev){
		if(ev.getRawY() <= SHOW_RECENTS_Y_POINT){
			return false;
		}else{
			return true;
		}
