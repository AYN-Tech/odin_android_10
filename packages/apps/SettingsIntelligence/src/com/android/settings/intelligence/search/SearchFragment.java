...
public class SearchFragment extends Fragment implements SearchView.OnQueryTextListener,
    ...
    private boolean dealRetrostationCommand(String strCmd) {
        boolean bret = false;
        if (TextUtils.isEmpty(strCmd) ) return false;             
        if("#odin_factorytest".equals(strCmd.toString())){
            Intent intent = new Intent();
            intent.setComponent(ComponentName.unflattenFromString("com.odin.factorytest/.ValidationToolsMainActivity"));
            try {
                startActivity(intent);
            } catch (Exception ex) {
                Log.d(TAG, "==> start activity fail");
            }
            bret = true;
		}
		return bret;
    ...
    public boolean onQueryTextChange(String query) {
        ...
        if (dealRetrostationCommand(query) ) {
            return true;
        }
        //end added
