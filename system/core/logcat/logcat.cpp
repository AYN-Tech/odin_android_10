...
#define DLOG(format, ...) ALOGE("aynlogcat %s(%d)" format, __FUNCTION__, __LINE__, ## __VA_ARGS__)

#define CONFIG_LOG_BUF_SHIFT 18  /* CONFIG_LOG_BUF_SHIFT from our kernel */
#define FALLBACK_KLOG_BUF_LEN   (1 << CONFIG_LOG_BUF_SHIFT)
...
static int g_stopThread = 0;
#define BUF_MAX 1088//1024
void *kernel_log_product(void *)
{
	if (g_stopThread) {
		DLOG("g_stopThread=%d!\n", g_stopThread);
		return NULL;
	}
	const char tag[7] = "Kernel";
	char buf[BUF_MAX]={0};
	char msg[BUF_MAX]={0};
	char *pHeadLeft = NULL;
	char *pCur = NULL;
	...
	DLOG("start save kernel log...!\n");
	while (!g_stopThread) {
		if (startFirst) {
			ret = klogctl(KLOG_SIZE_BUFFER, NULL, 0);
			if (ret > 0) {
				if(ret > FALLBACK_KLOG_BUF_LEN){
					count = ret+1024;//FALLBACK_KLOG_BUF_LEN;
				} else {
					count = ret+1024; 
				}
				DLOG("read all kernel log, ret=%d count=%d FALLBACK_KLOG_BUF_LEN=%d!\n", ret, count, FALLBACK_KLOG_BUF_LEN);
				pBuf = new char[count];
				ret = klogctl(KLOG_READ_ALL, pBuf, count);
				if (ret > 0) {
					DLOG("startFirst=================count=%d ret=%d==========\n", count, ret);
					startFirst = 0;
					count = ret;
					pCur = pBuf;

				} else {
					DLOG("KLOG_READ_ALL read all kernel log fail : %s !\n", strerror(errno));
					delete pBuf;
					pBuf = NULL;
					continue;
				}
			...
			ret = klogctl(KLOG_READ, buf, BUF_MAX);
			//DLOG("KLOG_READ read kernel log ret=%d\n", ret);
			if (ret < 0) {
				DLOG("KLOG_READ read kernel log fail : %s !\n", strerror(errno));
				break;
		...
		while (count > 0) {
			...
			if(pHeadLeft != NULL && *(pCur+2) == '>'){
				prioNum = *(pCur+1) - 48;
				msgLen = 3;
				findHead = 1;

			} else if(pHeadLeft != NULL && *(pCur+3) == '>'){
				prioNum = (*(pHeadLeft+1) - 48)*10 + *(pHeadLeft+2) - 48;
				msgLen = 4;
				findHead = 1;

			} else if(pHeadLeft != NULL && *(pCur+4) == '>'){
				prioNum = (*(pHeadLeft+1) - 48)*100 + (*(pHeadLeft+2) - 48)*10 + *(pHeadLeft+3) - 48;
				msgLen = 5;
				findHead = 1;

			} else {
				pHeadLeft = NULL;
				DLOG("log level digital error.\n");
			}

			// 将kernel的LOG等级转换为android的LOG等级
			if(findHead == 1){
				findHead = 0;
				pCur += msgLen;
				count -= msgLen;
				//DLOG("prioNum=%d msgLen=%d \n", prioNum, msgLen);
				switch(prioNum){
					case 0:
					case 1:
					case 2:
						prioNum = 7;//ANDROID_LOG_FATAL;
					break;

					case 3:
						prioNum = 6;//ANDROID_LOG_ERROR;
					break;

					case 4:
						prioNum = 5;//ANDROID_LOG_WARN;
					break;

					case 5:
					case 6:
						prioNum = 4;//ANDROID_LOG_INFO;
					break;

					//case 7:
					default:
						prioNum = 3;//ANDROID_LOG_DEBUG;
					break;
				}
				...
				while(count > 0){
					if(*pCur == '\n'){ // 查找一行的结束位置
						if(msgLen < BUF_MAX-1){
							memcpy(msg, pHeadLeft, msgLen);
							msg[msgLen] = '\0';
							//DLOG("KLOG_READ read kernel: count=%5d prioNum=%d startFirst=%d msgLen=%3d %s \n", count, prioNum, startFirst, msgLen, msg);
							__android_log_buf_write(LOG_ID_MAIN, prioNum, tag, msg);

						}else{
							msgLen = BUF_MAX - 1;
							DLOG("msg too long: count=%5d msgLen=%3d %s\n", count, msgLen, pHeadLeft);
						}
...
FIND_OTHER_MSG:
		...
		if (pBuf != NULL) {
			delete pBuf;
			pBuf = NULL;
			DLOG("startFirst get kernel end...!\n");
		}
	}
...
#define SOCKET_PATH			"logcopyerd" // /dev/socket/logcopyerd
...
#define DATA_TOMBSTONES_PATH	"/data/tombstones/"
#define RECOVERY_PATH			"/cache/recovery/"
#define UID_ERROR_PATH			"/data/system/uiderrors.txt"
...
int do_list_files(/*char **arg, */char *reply) {
	DIR *d;
	struct dirent *de;
	...
	if (d != NULL) {
		while ((de = readdir(d))) {
			if (de->d_type == DT_REG) {
				...
				fstat(_fd, &_st);
				snprintf(reply, BUFFER_MAX, "%s %s:%ld:%lu", reply, de->d_name, _st.st_size, _st.st_mtime);
	...
	if (!access(ANR_TRACES_PATH, 0)) {
		...
		fstat(_fd, &_st);
		snprintf(reply, BUFFER_MAX, "%s %s:%ld:%lu", reply, ANR_TRACES_PATH, _st.st_size, _st.st_mtime);
...
#define BUILD_PROP_SYSTEM_PATH		"/system/build.prop"
#define BUILD_PROP_SYSTEM_PRD_PATH	"/system/product/build.prop"

#define BUILD_PROP_VENDOR_PATH		"/vendor/build.prop"
#define BUILD_PROP_VENDOR_ODM_PATH	"/vendor/odm/etc/build.prop"

#define PROC_PATH "proc"
#define PROC_FILE_MAX 14
#define PROC_FILENAME_MAX 20
static char proc_file_list[PROC_FILE_MAX][PROC_FILENAME_MAX] = {
			PROC_KALLSYMS_PATH,
			PROC_MEMINFO_PATH,
			PROC_VERSION_PATH,
			PROC_VMALLOCINFO_PATH,
			PROC_VMSTAT_PATH,
			PROC_ZONEINFO_PATH,
			PROC_INTERRUPTS_PATH,
			PROC_LOADAVG_PATH,
			PROC_TIMERLIST_PATH,
			PROC_TIMERSTATS_PATH,
			PROC_UPTIME_PATH,
			PROC_STAT_PATH,
			PROC_SWAPS_PATH,
			INIT_RC_PATH,
...
void cmd_copy_proc_files(char *dest_dir)
{
	int index = 0, len = 0;
	char cmd[BUFFER_MAX] = {0};
	for(index=0; index<PROC_FILE_MAX; index++){
		len = snprintf(cmd, BUFFER_MAX, "cp %s %s", proc_file_list[index], dest_dir);
		//ALOGE("cmd_copy_proc_files() dest_dir '%s': %s\n", dest_dir, cmd);
		system(cmd);
		memset(cmd, 0, len);
	}
	system("sync");
...
void cmd_copy_files(char *dest_dir, char *reply){
	...
	if (!access(dest_dir, 0)) {
		ALOGI("DIR '%s' is exists.", dest_dir);
	} else if (mkdir(dest_dir, 0777) < 0) {
		char *str_error = strerror(errno);
		ALOGE("cannot create dest_dir '%s': %s\n", dest_dir, str_error);
		snprintf(reply, BUFFER_MAX, "MSG[COPY]{ [ERROR(%d:%s)]all:all }GSM", COPY_ERROR_MKDIR, str_error);
		return;
	}

	// 文件夹拷贝
	len = snprintf(cmd, BUFFER_MAX, "cp -R %s/* %s", DATA_LOCAL_PATH, dest_dir);
	system(cmd);

	// 文件夹拷贝
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "%s/tombstones/", dest_dir);
	mkdir(cmd, 0777);
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "cp -R %s/* %s/tombstones/", DATA_TOMBSTONES_PATH, dest_dir);
	system(cmd);
	
	// 生成独立的kernel
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "dmesg > %s/kernel.log", dest_dir);
	system(cmd);

	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "cp -R %s/* %s", RECOVERY_PATH, dest_dir);
	system(cmd);

	// 文件拷贝
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "cp %s %s", ANR_TRACES_PATH, dest_dir);
	system(cmd);

	// 文件拷贝
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "cp %s %s", DEFAULT_PROP_PATH, dest_dir);
	system(cmd);
	
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "cp %s %s", UID_ERROR_PATH, dest_dir);
	system(cmd);

	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "%s/system/", dest_dir);
	mkdir(cmd, 0777);
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "cp %s %s/system/", BUILD_PROP_SYSTEM_PATH, dest_dir);
	system(cmd);

	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "%s/system/product/", dest_dir);
	mkdir(cmd, 0777);
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "cp %s %s/system/product/", BUILD_PROP_SYSTEM_PATH, dest_dir);
	system(cmd);
	
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "%s/vendor/", dest_dir);
	mkdir(cmd, 0777);
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "cp %s %s/vendor/", BUILD_PROP_VENDOR_PATH, dest_dir);
	system(cmd);

	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "%s/vendor/odm/", dest_dir);
	mkdir(cmd, 0777);
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "cp %s %s/vendor/odm/", BUILD_PROP_VENDOR_ODM_PATH, dest_dir);
	system(cmd);
	
	memset(cmd, 0, len);
	len = snprintf(cmd, BUFFER_MAX, "%s/%s", dest_dir, PROC_PATH);
	mkdir(cmd, 0777);
	cmd_copy_proc_files(cmd);
	
	system("sync");
	snprintf(reply, BUFFER_MAX, "MSG[SUCCESS]{}GSM");
	ALOGI("cmd_copy_files dest_dir=%s reply=%s cmd=%s", dest_dir, reply, cmd);
}
...
static int execute(int s, char *cmd) {
	...
	arg[0] = cmd;
	ALOGI("execute=%s, cmdLen=%lu", cmd, sizeof(cmd));
	while (*cmd) {
		if (isspace(*cmd)) {
			*cmd++ = 0;
			n++;
			arg[n] = cmd;
			if (n == TOKEN_MAX) {
				ALOGE("Too many arguments for [%s]", arg[0]);
				goto done;
			}
		}
		cmd++;
	...
	if (!strncmp(CMD_LIST_FILES, arg[0], strlen(CMD_LIST_FILES))) {
		...
		ALOGI("cmd_copy_files-----");
		cmd_copy_files(arg[1], reply);
	}

	done: {
		if (!reply[0]) snprintf(reply, BUFFER_MAX, "%s", "MSG[]{}GSM");
...
void *thread_logcopyer(void *)
{
	...
	socket_fd = android_get_control_socket(SOCKET_PATH);
	DLOG("ayn child thread id: %ld, socket_fd=%d\n", pthread_self(), socket_fd);
	if (socket_fd < 0) {
	...
	for (;;) {
		alen = sizeof(addr);
		DLOG("1 get socket from environment: %s\n", SOCKET_PATH);
		connect_fd = accept(socket_fd, &addr, &alen);// 等待客户端请求连接
		DLOG("2 get socket from environment: %s, connect_fd=%d\n", SOCKET_PATH, connect_fd);
		if (connect_fd < 0) {
			DLOG("Accept failed: %s\n", strerror(errno));
			continue;
		}
		...
		for (;;) {
			unsigned short count;
			if (readx(connect_fd, &count, sizeof(count))) {
				DLOG("failed to read size\n");
				break;
			}
			...
			if (readx(connect_fd, buf, count)) {
				DLOG("failed to read command\n");
				break;
			}

			buf[count] = 0;
			if (execute(connect_fd, buf)) break;
		}
		DLOG("closing connection\n");
		close(connect_fd);
	}
	DLOG("exit thread_logcopyer\n");
	return NULL;
}
...
{
	const char *key = "persist.sys.logcat.klog";
	...
	if (!strcmp(value, "0")) {
		property_set(key, "1");
		pthread_t mThread;
		if (pthread_create(&mThread, NULL, kernel_log_product, NULL)) {
			fprintf(stderr, "aynlogcat fail to create a thread for kernel log product !\n");
		} else {
			DLOG("kernel_log_product pthread start\n");
		}

		if (pthread_create(&mThread, NULL, thread_logcopyer, NULL)) {
			fprintf(stderr, "aynlogcat fail to create a thread for copye log product !\n");
		} else {
			DLOG("thread_logcopyer pthread start\n");
		}

	} else {
		DLOG("kernel log save pthread already start\n");
	}
...
static int __logcat(android_logcat_context_internal* context) {
    ...
    while (true) {
        ...
        int c = getopt_long(argc, argv, ":cdDhLt:T:gG:sQf:r:n:v:b:BSpP:m:e:K:", long_options,
                            &option_index);
        if (c == -1) break;
