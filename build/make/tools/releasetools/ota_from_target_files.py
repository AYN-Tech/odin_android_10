...
def WriteCompatibility(output_zip,target_info):
  compatibility = target_info.GetBuildProp("rs.firmware.compatibility")
  hardware = target_info.GetBuildProp("rs.hardware.version")
  date = target_info.GetBuildProp("ro.build.date.utc")
  
  common.ZipWriteStr(output_zip, "compatibility_info", "rs.firmware.compatibility = "+compatibility+"\nrs.hardware.version = "+hardware+"\nro.build.date.utc = "+date)
...
def WriteABOTAPackageWithBrilloScript(target_file, output_file,
  ...
  if source_file is not None:
    ...
    target_info = BuildInfo(OPTIONS.info_dict, OPTIONS.oem_dicts)
    source_info = None
    WriteCompatibility(output_zip,target_info)
